class Carousel {
  ...
  _initListeners(){
    ...
    this._buttonsListener();
  }

  _buttonsListener(){
    this.slideButton = document.querySelectorAll('.slide__button');

    for(let i = 0; i < this.slideButton.length; i++){
      this.slideButton[i].addEventListener('mouseenter', this._pause.bind(this));
      this.slideButton[i].addEventListener('mouseleave', this._play.bind(this));
    }
  }
