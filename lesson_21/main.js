 ...
 */
replaceCSSComments = function(str){
 ...
 */
createLinksFromDomains = function(str){
  let reg = new RegExp('(http:|https:)\\/\\/(?!:\\/\\/)([a-zA-Z0-9-_]+\\.)*[a-zA-Z0-9-_]+\\.[a-zA-Z]{2,5}', 'gi');
  str.match(reg).forEach((i) => str = str.replace(i, `<a href="${i}" target="_blank">${i.replace(/^(http:|https:)\/\//gi, '')}</a>`));
  return str;
}
